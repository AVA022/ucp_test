...
static int free_buffer_chunk(int chunk_num, enum buffer_type type)
{
    int *buffer;
    switch (type)
    {
        ...
        for(int i = 0; i < num_loop; i ++){
            ...
            for(const auto& step: tb->steps){
                if(step->depid != -1){
                    ...
                    log_trace("rankid:%d tbid:%d wait for %s\n", world_rank, tb_id, hash_key.c_str());
...
int free_buffer_pool(){
    auto& rank_intr = xmlparser.ranks[world_rank];
    if(rank_intr->i_chunks != 0){
        if(free_buffer_chunk(rank_intr->i_chunks, INPUT) != 0){
            log_error("init_buffer_chunk failed!\n");
            return -1;
        }
    }
    ...
    if(rank_intr->o_chunks != 0){
        free_buffer_chunk(rank_intr->o_chunks, OUTPUT);
    }

    if(rank_intr->s_chunks != 0){
        free_buffer_chunk(rank_intr->s_chunks, SCRATCH);
    }
    return 0;
}
...
int parse_arguments(int argc, char *argv[], char **filename, int *buffer_size_int) {
    ...
    while ((opt = getopt(argc, argv, "l:si:")) != -1) {
        switch (opt) {
        case 'i':
...
int main(int argc, char **argv) {
    ...
    //log_info("start allreduce test");

    for(int i = 256; i <= 1024 * 1024; i = i * 2){
        init_buffer_pool(i);
        total_elapsed = std::chrono::duration<double, std::milli>::zero();
        //log_info("%f", total_elapsed.count());
        // for(int i = 0; i < xmlparser.ranks[world_rank]->tbs.size(); i++){
        //         sem_post(&sem_4_perftest);
        // }
        for(int i = 0; i < num_loop; i ++) {
            for(int i = 0; i < xmlparser.ranks[world_rank]->tbs.size(); i++){
        ...
        if(world_rank == 0){
            log_info("[%dKB] rank id : %dtotal time cost: %f ms  num_loop = %d",i * 4 / 1024,world_rank , total_elapsed.count(), num_loop);
            log_info("[%dKB] average time cost: %f ms",i * 4 / 1024, total_elapsed.count() / num_loop);
        }
        free_buffer_pool();
    }
